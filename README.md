...
M11 adds the React developer dashboard for M10 observability APIs: application-scoped overview, events and event detail, deliveries and delivery detail with attempt history. It uses opaque cursor pagination, route-based application selection, explicit redacted DTO fields, and safe text-only JSON payload rendering. Endpoint/API-key management and delivery mutation UI remain out of scope. M11.2 adds the public frontend landing page at `/`; it requires no dashboard data and links visitors to the existing sign-in flow.
